...
inline size_t precedence(const char op)
{
    ...
    if (op == '^')
       return 3;
    throw std::runtime_error {string{"존재하지 않는 연산자 입니다: "} + op};
}
...
double execute(std::stack<char>& ops, std::stack<double>& operands)
{
    ...
    case '^':
      ...
      break;
    default:
       throw std::runtime_error {string{"존재하지 않는 연산자 입니다: "} + ops.top()};
    }
    ops.pop();
...
int main()
{
    ...
    string exp;
    std::cout << "An Arithmetic expression can include the operators +, -, *, /, and ^ fir exponentiation." << std::endl;
    try
    {
        while (true)
        {
